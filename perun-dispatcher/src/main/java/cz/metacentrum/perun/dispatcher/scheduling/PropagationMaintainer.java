...
public class PropagationMaintainer extends AbstractRunner {
	...
	private void rescheduleDoneTasks() {
		...
		for (Task task : schedulingPool.getTasksWithStatus(TaskStatus.DONE)) {
			...
			LocalDateTime twoDaysAgo = Instant.ofEpochMilli(System.currentTimeMillis() - 1000 * 60 * 60 * 24 * 2).atZone(ZoneId.systemDefault()).toLocalDateTime();
	...
	private void rescheduleErrorTasks() {
		...
		for (Task task : schedulingPool.getTasksWithStatus(TaskStatus.ERROR, TaskStatus.GENERROR, TaskStatus.SENDERROR)) {
			...
			if (task.getEndTime() == null) {
				...
				LocalDateTime endTime = Instant.ofEpochMilli(System.currentTimeMillis() - ((task.getDelay() + 1) * 60000)).atZone(ZoneId.systemDefault()).toLocalDateTime();
				task.setEndTime(endTime);
			}

			int howManyMinutesAgo = (int) (System.currentTimeMillis() - task.getEndTime().atOffset(ZoneOffset.UTC).toInstant().toEpochMilli()) / 1000 / 60;
	...
	private void endStuckTasks() {
		...
		for (Task task : suspiciousTasks) {
			...
			int howManyMinutesAgo = (int) (System.currentTimeMillis() - (laterTimestamp == null ? soonerTimestamp : laterTimestamp).atOffset(ZoneOffset.UTC).toInstant().toEpochMilli()) / 1000 / 60;
