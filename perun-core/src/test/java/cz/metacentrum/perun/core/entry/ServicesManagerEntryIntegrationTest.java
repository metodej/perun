...
public class ServicesManagerEntryIntegrationTest extends AbstractPerunIntegrationTest {
	...
	public void getFlatDataWithoutExpiredUsers() throws Exception {
		...
		Member member5 = setUpMemberFromVo2(vo2);
		...
		List<ServiceAttributes> facilityElements = facilities.get(0).getChildElements();
		List<ServiceAttributes> users = facilityElements.get(1).getChildElements();
		assertEquals("There should be all 3 users (possibly 5 because 2 are in 2 resources connected to our facility)", 3, users.size());
		...
		assertEquals("There should be only 2 users (possibly 3 because 1 is in 2 resources connected to our facility)", 2, users.size());
	}
	...
	private Member setUpMemberFromVo2(Vo vo2) throws Exception {
